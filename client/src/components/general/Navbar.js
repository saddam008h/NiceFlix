  ...
  return (
    <>
      ...
      >
        ...
        {isInputOpen && (
       <div id="searchIcon" ref={searchBarRef}  className="search-input-container flex absolute top-3 right-20 xl:top-4 xl:right-36 border border-solid border-white transition-all duration-5000 transform translate-x-full ">
          ...
          />{ searchParam.get('q') && <i onClick={()=> {setSearchParam('')}} class='bx bx-x bg-gray-900 cursor-pointer hover:text-white opacity-90 xl:opacity-80 top-7 pt-1 xl:pt-2 text-3xl'></i>}
        </div>
      )}
        <Link to="/account">
          {!user?
          ...
          :<div className="xl:text-2xl xl:mt-3  text-white font-extrabold">{user.firstName}</div>  }
        </Link>
      </div>
      <div className="relative">
        <div className={` ${isClicked ? 'flex':'hidden'} flex-col items-center gap-6 border border-t-white border-t-4 border-gray-600 py-5 absolute top-14 z-50 left-5 bg-gray-800 opacity-90 w-48`}>
          <div className="arrow text-white cursor-pointer "></div>
          <Link to="/"> <div className="hover:text-white cursor-pointer">Home</div> </Link>
          <Link to="/movies"><div className="hover:text-white cursor-pointer">Movies</div></Link>
          <Link to="/shows"><div className="hover:text-white cursor-pointer">TV Shows</div></Link>
        </div>
      </div>
    </>
  );
